...
function installMenu(applyLocale, togglePet) {
  ...
  const languageSubmenu = [
    {
      ...
      type: 'radio',
      checked: locale === 'zh',
      click: () => { switchLocale('zh', applyLocale, togglePet) },
    },
    {
      label: t.english,
      type: 'radio',
      checked: locale === 'en',
      click: () => { switchLocale('en', applyLocale, togglePet) },
    },
  ]
  ...
  if (!mac) {
    ...
    template.push({
      label: t.view,
      submenu: [petItem, { type: 'separator' }, { label: t.quit, role: 'quit' }],
    })
  }
 ...
 */
function switchLocale(next, applyLocale, togglePet) {
  if (currentLocale() === next) return
  writePrefs({ ...readPrefs(), locale: next })
  installMenu(applyLocale, togglePet)
