 ...
 * 旁观其他会话，攒出一份"刚才发生了什么"。
 *
 * 陪伴助手不是聊天对象，而是**旁观者**：你和主界面的智能体干活，它在一边看着，等那边收
 ...
 * 空的，而她会一本正经地报告"那位智能体一个字没吐"。
 *
 * 推理段（`reasoning`）一律丢掉。那是模型的草稿，既不是给人看的，也不该成为另一个
 * 模型总结的输入 —— 让她去转述别人的思考过程，只会得到一段更含糊的思考过程。
 *
 * @module pet-observer
 */
 ...
 *
 * @param {object} deps
 * @param {(sessionId: string) => boolean} deps.isPetSession 判断某个会话是不是陪伴助手自己的
...
function createPetObserver({ isPetSession, onDigest, now = Date.now }) {
  ...
  const observe = (frame) => {
    ...
    if (frame.type === 'session/event') {
      ...
      // 陪伴助手自己那条会话不进素材：它说的话是**结果**，再喂回去就成了自己总结自己。
