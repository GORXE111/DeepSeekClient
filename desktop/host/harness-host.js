...
const { unpackedPath } = require('./asar-path.js')
...
/** 结构化消息走 IPC 通道；日志另走 stdio。 */
const say = (type, payload) => { process.send({ type, payload }) }
...
function installStubIntoProfile(dshHome) {
  ...
  const source = unpackedPath(path.join(DESKTOP, 'host', 'plugins', 'webserver-ipc'))
