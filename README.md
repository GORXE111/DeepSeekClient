...
desktop/     Electron 壳：窗口、托盘、通知、桌面陪伴助手
tools/       构建脚本
...
桌面角落常驻一个像素小人（默认关闭，视图菜单里开）：智能体干完一轮她会来报一声、
点开可以直接问小问题、按住能拖着走。

**两位可选**，在 设置 → 通用设置 → 桌面陪伴助手 里换：

| | 动画 | 说话风格 |
|---|---|---|
| **MIKU** | 8 套 | 明快、短促、可爱 |
| **庄方宜** | 12 套（含雷法、青霆剑、打坐） | 沉稳、干练、话不多 |

两位**各有各的会话与上下文**：跟 MIKU 说过的话不会出现在庄方宜那边，换回来时也接着
